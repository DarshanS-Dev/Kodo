        ...
        {CODE_LINES.map((line, idx) => {
          ...
          return (
            <div key={idx} className={cn(
              ...
              <span className={cn(
                isErr && phase === "paused" ? "underline decoration-wavy decoration-[#F87171]" : ""
              )}>
                {renderLine(line, idx)}
              </span>
              {isActive && (
        ...
        <div ref={consoleRef} className="px-3.5 py-2 max-h-[110px] overflow-y-auto bg-[#0A0B10] leading-[1.8]">
          {consoleLogs.map((log, i) => (
            <motion.div
              ...
              style={{ color: CONSOLE_COLORS[log.type] || "#cbd5e1" }}
      ...
      {pts.map(p => (
        <div
          ...
          className={cn(
            ...
            p.filled ? "bg-[rgba(107,26,26,0.35)]" : "bg-transparent border-[1.5px] border-[rgba(107,26,26,0.28)]"
          )}
          style={{
            left: `${p.x}%`,
            top: `${p.y}%`,
...
export default function LoginPage() {
  ...
  return (
    <div className={cn(dmSans.className, "min-h-screen flex items-center justify-center bg-[#F2EDE4] relative overflow-hidden")}>
      {/* Background Decor */}
      <div className="fixed bottom-[-12vh] right-[-12vw] w-[65vw] h-[65vw] bg-[#6B1A1A] rounded-full z-0" />
      <div className="fixed top-[-8vw] left-[-8vw] w-[26vw] h-[26vw] bg-[rgba(107,26,26,0.1)] rounded-full z-0" />
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-11 p-6">
        ...
        {/* ── Login Card ── */}
        <div ref={cardRef} className="bg-[#F2EDE4]/80 backdrop-blur-3xl border border-[#6B1A1A]/15 rounded-[28px] p-11 pt-12.5 w-[360px] shadow-[0_8px_40px_rgba(107,26,26,0.13)]">
          <h1 className={cn(playfair.className, "text-[2rem] font-bold text-[#6B1A1A] text-center mb-8 tracking-tight")}>
            Login Now
          </h1>
          ...
          <form onSubmit={handleLogin} className="space-y-3.5">
            {[
              ...
              <input key={k} type={type} placeholder={ph}
                ...
                className={cn(
                  "w-full px-4.5 py-3 rounded-xl bg-white/60 border outline-none text-[0.9rem] text-[#2A1010] transition-all duration-200",
                  focused === k ? "border-[#8B2222] ring-3 ring-[#6B1A1A]/10" : "border-[#6B1A1A]/20"
                )}
              />
            ))}

            <button type="submit" disabled={loading} className={cn(
              "w-full py-3.5 bg-[#6B1A1A] disabled:bg-[#8B2222] text-[#F2EDE4] rounded-xl font-semibold text-[0.92rem] tracking-widest uppercase transition-colors shadow-[0_4px_16px_rgba(107,26,26,0.25)] flex items-center justify-center gap-2.5",
              loading ? "cursor-not-allowed" : "cursor-pointer hover:bg-[#8B2222]"
            )}>
              {loading && (
                <div className="w-4 h-4 border-2 border-[#F2EDE4]/30 border-t-[#F2EDE4] rounded-full animate-spin" />
              )}
          ...
          <div className="flex items-center gap-2.5 my-4 text-[#5A3333] text-[0.82rem] opacity-70">
            <div className="flex-1 h-[1px] bg-[#6B1A1A]/18" />
            Or login with
            <div className="flex-1 h-[1px] bg-[#6B1A1A]/18" />
          </div>
          ...
          <div className="flex gap-3 mb-5.5">
            {[
              ...
              <button key={label} type="button" className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-[#6B1A1A]/20 bg-white/55 text-[0.87rem] font-medium text-[#2A1010] cursor-pointer hover:bg-white/70 transition-colors">
                {icon}{label}
              </button>
            ))}
          </div>

          <p className="text-center text-[0.84rem] text-[#5A3333] opacity-85">
            Not a member?{" "}
            <a href="/register" className="text-[#6B1A1A] font-semibold hover:underline transition-all">Register Now</a>
          </p>
        </div>

        {/* ── Debugger Panel ── */}
        <div ref={sideRef} className="flex flex-col items-center gap-3.5">
          ...
          <div className="text-[#F2EDE4]/40 text-[0.7rem] tracking-wider">
